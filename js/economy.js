...
function addMoney(n,worldPos){
  ...
  if(n>0 && typeof trackMission==='function') trackMission('cashEarned',n);
...
const SHOP_ITEMS=[
  ...
  { id:'minigun', name:'Minigun',      ico:'⚙️', price:1500, desc:'120 rnd · shreds crowds' },
  { id:'rocket',  name:'RPG Launcher', ico:'🚀', price:1800, desc:'Explosive · big AoE' },
  { id:'bribe',   name:'Lose the Heat',ico:'🚔', price:400, desc:'Clear your wanted level' },
];
const SHOP_WEAPONS=['rifle','shotgun','sniper','minigun','rocket'];
...
function renderShop(){
  ...
  SHOP_ITEMS.forEach(it=>{
    const owned = SHOP_WEAPONS.includes(it.id) && Game.weapons.find(w=>w.id===it.id)?.owned;
...
function buyItem(it){
  ...
  switch(it.id){
    ...
    case 'medkit': Game.playerData.hp=Math.min(Game.maxHP||100,Game.playerData.hp+40); refreshHP(); break;
    ...
    case 'rifle': case 'shotgun': case 'sniper': case 'minigun': case 'rocket': ownWeapon(it.id); break;
