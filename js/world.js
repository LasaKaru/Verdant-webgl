...
function buildTerrain(scene){
  // scale tessellation with world size so facets stay a sensible size on the
  // much larger map (capped so load + GPU stay reasonable).
  const facet = Game.settings.quality==='low'?5.4 : Game.settings.quality==='high'?3.0 : 4.0;
  const subs = Math.min(260, Math.round(WORLD*2/facet));
...
function buildWorld(scene,density,shadows){
  ...
  const D=density;
  // scale counts with the world's area so the big map doesn't feel empty,
  // but cap each so the instance/shadow/collision load stays sane.
  const AREA=(WORLD/130)*(WORLD/130);
  // distribute by biome — sample random points, decide what to place
  const treeTarget=Math.min(700,Math.floor(D*AREA*0.62)), rockTarget=Math.min(340,Math.floor(D*AREA*0.30)),
        bushTarget=Math.min(380,Math.floor(D*AREA*0.34)), flowerTarget=Math.min(240,Math.floor(D*AREA*0.22)),
        houseTarget=Math.min(60,Math.floor(D*AREA*0.04)), ruinTarget=Math.min(46,Math.floor(D*AREA*0.03));
  ...
  // lighter fog so you see deep into the big world, while the far boundary
  // still fades out — giving the map an open, endless feel.
...
function updateSky(){
  ...
  if(typeof updateExtraLights==='function') updateExtraLights(sunH);
}
