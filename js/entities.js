...
function damageEnemy(e,dmg,crit,forceKill){
  if(e.dead) return;
  // a clean shot drops any non-boss hostile instantly; bosses take real damage
  if(forceKill && !e.boss) e.hp=0; else e.hp-=dmg;
...
function killEnemy(e){
  ...
  const cash = Math.round((e.boss?600 : e.type==='brute'?80 : e.type==='shooter'?50 : 25)*((typeof perkCashMult==='function')?perkCashMult():1));
  ...
  if(typeof trackMission==='function'){ trackMission('kills',1); if(e.boss) trackMission('bosses',1); }
  sfx('kill');
  if(e.boss){
  ...
  updateHUD();
  if(typeof addXP==='function') addXP(e.boss?220 : e.type==='brute'?30 : e.type==='shooter'?18 : 12);
...
const ITEM_DEFS={
  ...
  minigun:{ico:'⚙️',nm:'MINIGUN'}, rocket:{ico:'🚀',nm:'RPG'},
  railgun:{ico:'⚡',nm:'RAILGUN'}, flame:{ico:'🔥',nm:'FLAME'}, goldengun:{ico:'✨',nm:'GOLD GUN'},
};
