...
async function boot(){
  ...
  cam.fov=Game.settings.fov; cam.minZ=0.15; cam.maxZ=520;
  ...
  function finishLoad(){
    Game.playerData={hp:100,stamina:100,armor:0};
...
function updateGame(dt,now){
  ...
  const sprinting=k['shift']&&Game.playerData.stamina>0&&moving&&!Game.crouching;
  let speed=6.2*(sprinting?1.7:1)*(Game.aiming?0.5:1)*(Game.crouching?0.5:1);
  ...
  /* waves */
  if(Game.waveActive){
    if(Game.enemiesToSpawn>0 && Game.enemies.length<9 && Math.random()<0.05){ spawnEnemy(Game.scene,Game.settings.shadows); Game.enemiesToSpawn--; }
  ...
  const ch=$('crosshair');
  if(ch){ const tgt=(typeof screenTarget==='function')?screenTarget(220):null; ch.classList.toggle('hot',!!tgt); }
}
...
function resetGame(){
  ...
  Game.score=0; Game.wave=0; Game.waveActive=false; Game.betweenWaves=false;
...
function gameOver(){ Game.aiming=false; Game.mouseDown=false; document.exitPointerLock&&document.exitPointerLock();
  $('finalScore').textContent=Game.score; $('finalWave').textContent='REACHED WAVE '+Game.wave; setState('over'); }
...
function bindInput(){
  ...
  window.addEventListener('keydown',e=>{
    ...
    if(Game.state==='playing'){
      if(key>='1'&&key<='4'){ switchWeapon(+key-1); }
      if(key==='r') reload();
      ...
      if(key==='escape'){ if(Game.mapOpen){ closeMap(); } else if(Game.shopOpen){ closeShop(); } else if(Game.garageOpen){ closeGarage(); } else if(Game.contractsOpen){ closeContracts(); } else pauseGame(); }
  ...
  document.addEventListener('mousemove',e=>{
    ...
    if(document.pointerLockElement===canvas){
      ...
      Game.pitch=clamp(Game.pitch+e.movementY*s,-0.35,1.32);
...
function overlayOpen(){
  return Game.mapOpen||Game.shopOpen||Game.garageOpen||Game.contractsOpen||$('inv').classList.contains('show');
}
